# 研發部 Codex＋Antigravity AI 使用報告規範 v2
...
- 時區：Asia/Taipei
- 部門：研發部
- 來源：該員工本機 Codex session，以及存在且可讀取時的 Google Antigravity conversation／brain 紀錄
...
1 筆＝1 筆去重後、與工作相關的 Codex 或 Antigravity 任務／session。不是正式專案數、工時、Token 或交付成果數。

同一 session 只有在確定是內部 companion 或完全重複時才排除。兩平台若只是同一任務的連續交接且沒有獨立成果，只計一次；實際完成不同除錯、重構、查核或交付時仍分開保留。相似但目標不同的工程工作不得過度合併。
...
- Codex：將納入工作 session 期間內 `token_count` 事件的 `last_token_usage` 相加，input 可能包含 Cached Input。
- Antigravity：`totalTokens = inputTokens + outputTokens + cacheWriteTokens + cacheReadTokens`；`thinkingOutputTokens` 與 `responseOutputTokens` 是 output 子集，不得重複加總。
- 合併 Token 供部門額度規劃，不等同財務帳單、剩餘配額或績效。

### 使用天數與活躍互動

- 使用天數：兩平台納入工作 session 的 Asia/Taipei 日曆日聯集去重。
- 適用天數：從 `max(使用者指定起日, 首筆工作相關 Codex 日期)` 到指定迄日的日曆天數；0 筆時為 0。
- KPI 顯示 `使用天數／適用天數`，避免較晚開始使用者被完整 25 天分母不公平解讀。
- 活躍互動：每個 session 以 15 分鐘間隔切分互動區段；每段至少 2 分鐘。跨平台有區段資料時取聯集並把重疊只計一次；缺少區段時才退回各平台加總並註明限制。這是估計，不是正式工時。

### Skill 頻率

- Skill 覆蓋率＝兩平台偵測到實際 `$skill-name`、`SKILL.md` 或明確 Skill 路徑使用的工作紀錄數／工作紀錄總數。
...
- `AI 證據評分`：由 Codex 與 Antigravity 工作 session 中可回溯的工作內容、成果與助益證據保守評定。平台角色、方案與配額切換本身不構成加分。
...
`sources` 為可選欄位以維持 Codex-only 舊資料相容；整合報告必須提供。`sources[].workRecordCount` 與 `sources[].tokens.totalTokens` 應分別加總等於 `metrics.workRecordCount` 與 `metrics.totalTokens`。`role` 只允許描述主要／備援流程，不顯示為績效高低。
